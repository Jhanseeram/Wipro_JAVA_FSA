...
export class FlightSearch implements OnInit {
  ...
  searchFlights(): void {
    ...
    // Store search data in DataSharingService
    const searchData = {
      from: this.fromLocation,
      to: this.toLocation,
      travelDate: `${this.departureDay} ${this.departureMonthYear}`,
      passengers: 1 
    };
    this.dataSharingService.setFlightSearchData(searchData);
  ...
  // Initialize date display with current date
  initializeDateDisplay(): void {
    const today = new Date();
    this.departureDay = today.getDate().toString();
    this.departureMonthYear = `${this.monthNames[today.getMonth()].substring(0, 3)}'${today.getFullYear().toString().substring(2)}`;
    this.departureWeekday = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][today.getDay()];
  }
