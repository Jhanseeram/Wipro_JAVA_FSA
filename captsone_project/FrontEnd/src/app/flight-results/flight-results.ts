...
@Component({
  ...
  templateUrl: './flight-results-styled.html',
...
export class FlightResults implements OnInit {
  ...
  trackByFlight(index: number, flight: Flight): string {
    return flight.flightNumber + flight.departureTime;
  }
  ...
  updateFlightResults(): void {
    ...
    this.flightService.getFlightsByRouteAndDate(
      ...
      next: (data) => {
        ...
        this.flights = data;
        // Immediately set filteredFlights to ensure display
        this.filteredFlights = [...this.flights];
        console.log('Immediately set filteredFlights:', this.filteredFlights.length);
      ...
      error: (err) => {
        ...
        console.log('Mock flights filtered:', this.flights);
        // Immediately set filteredFlights for mock data too
        this.filteredFlights = [...this.flights];
        console.log('Immediately set filteredFlights (mock):', this.filteredFlights.length);
        this.applyFilters(); // Apply filters after loading data
        // Force change detection for mock data too
        this.cdr.detectChanges();
      }
    });
  }
  ...
  applyFilters(): void {
    ...
    console.log('Filter values:', this.filters.map(f => ({ name: f.name, value: f.value })));
    console.log('Total flights before filtering:', this.flights.length);
    console.log('Flights data:', this.flights);
    
    // If no filters are active, show all flights
    if (!this.hasActiveFilters()) {
      this.filteredFlights = [...this.flights];
      console.log('No filters active, showing all flights:', this.filteredFlights.length);
      console.log('FilteredFlights details:', this.filteredFlights);
      // Force change detection
      this.cdr.detectChanges();
    ...
    console.log('Filtered flights:', this.filteredFlights.length);
  ...
  bookFlight(flight: Flight): void {
    // Store selected flight in DataSharingService
    const flightData = {
      flightNumber: flight.flightNumber,
      airlineName: flight.airline,
      airlineCode: flight.flightNumber.substring(0, 2), // Extract airline code (e.g., "AI" from "AI 785")
      aircraftType: 'Airbus A320 N', // Default aircraft type
      ...
      departure: {
        city: flight.departureCity || this.fromLocation,
        code: (flight.departureCity || this.fromLocation).toUpperCase().substring(0, 3),
        airport: 'Airport' // Default airport name
      },
      arrival: {
        city: flight.arrivalCity || this.toLocation,
        code: (flight.arrivalCity || this.toLocation).toUpperCase().substring(0, 3),
        airport: 'Airport' // Default airport name
      },
      departureDate: this.departureDate,
      flightType: flight.nonStop ? 'Non Stop' : '1 Stop'
    };
    
    this.dataSharingService.setSelectedFlight(flightData);
